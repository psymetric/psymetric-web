 ...
 *
 * SIL-13: Converts sensor observations and classifications into a chronological
 * sequence of SERP events for a KeywordTarget. Produces a minimal event stream
 * where duplicate consecutive classifications are collapsed.
 *
 * All computation is on-read; no writes, no mutations, no EventLog, no transactions.
 *
 * Signal derivation (all computed inline from the snapshot set, matching SIL-12):
 *   volatilityScore        -- computeVolatility().volatilityScore
 *   aiOverviewChurnCount   -- computeVolatility().aiOverviewChurn
 *   featureTransitionCount -- computeFeatureVolatility().summary.transitionCount
 *   intentDriftEventCount  -- computeIntentDrift().transitions.length
 *   averageSimilarity      -- mean combinedSimilarity across consecutive pairs
 *   dominanceDelta         -- |dominanceIndex(last snap) - dominanceIndex(first snap)|
 *
 * For the timeline, each snapshot gets its own classification computed using
 * cumulative signals up to and including that snapshot, then events are emitted
 * only on classification transitions.
 *
...
export async function GET(
  ...
  try {
    ...
    // ── Compute per-snapshot signals for timeline ─────────────────────────────
    // For each snapshot at index i, we compute signals over the window [0..i]
    // to classify what event was occurring at that point in time.
    ...
    console.error("GET /api/seo/keyword-targets/:id/event-timeline error:", err);
    return serverError();
  }
}
