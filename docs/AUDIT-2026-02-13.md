# PsyMetric Audit & Fixes — February 13, 2026

**Auditor:** Claude Opus 4.6  
**Scope:** Full codebase review — schema, API routes, dashboard, public pages, config  
...
**Priority:** 🔴 Critical  
**Issue:** Multiple API endpoints perform entity state changes and event logging as separate operations. If the event log write fails, you get a state change with no audit trail — breaking the core event-sourced invariant.  
**Files modified:**
...
**Files modified:**
- `src/app/api/entities/[id]/validate/route.ts` — removed `logEvent` import, replaced with direct Prisma call
**Note:** `src/lib/events.ts` is now dead code. Retained as pattern documentation; safe to delete.

### 8. Atomic Promote endpoint
**Priority:** 🔴 Critical (data integrity)  
**Issue:** PromoteModal in source-inbox made 3 sequential API calls (create entity → create relationship → update source status). If step 2 failed, orphaned entity. If step 3 failed, source status unchanged.  
**Files created:**
- `src/app/api/source-items/[id]/promote/route.ts`
**Files modified:**
- `src/app/dashboard/inbox/source-inbox.tsx` — PromoteModal now calls single `/api/source-items/[id]/promote`
...
- `src/app/api/entities/route.ts`
- `src/app/dashboard/entities/page.tsx`
**Fix:** Changed to `[createdAt desc, id desc]` — deterministic, matches pattern used by relationships/events/distribution/metrics.
...
- `GET /api/entities/[id]` added — CRUD contract complete
...
# Post-Audit Stabilization & Multi-Project Hardening — February 14, 2026
...
**Priority:** 🔴 Critical (isolation invariant)  
...
**Priority:** 🔴 Critical (graph integrity + isolation invariant)
...
Priority: 🔴 Critical (triage mutation boundary)
...
No project scoping — any SourceItem could be updated if ID was known.
...
Priority: 🔴 Critical (draft artifact boundary + isolation invariant)
...
Priority: 🔴 Critical (ingestion visibility + isolation invariant)
...
Priority: 🔴 Critical (lifecycle + cross-project mutation risk)
...
Enforced state transition (draft → publish_requested) only.
...
Priority: 🔴 Critical (lifecycle validation + event integrity)
...
Priority: 🔴 Critical (cross-project visibility risk)
...
Priority: 🔴 Critical (public cross-project leakage)
...
**Priority:** 🟠 High (invariant enforcement + regression prevention)
